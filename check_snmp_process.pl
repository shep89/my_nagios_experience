...
my $o_mem_sum=	undef;		# checks memory (sum)
...
-s
   makes an sum of memory used by process instead of max (by au20320)
-u, --cpu=WARN,CRIT
...
if (defined ($o_mem) ) {
 ...
 elsif (defined ($o_mem_sum)) {
		for (my $i=0; $i< $num_int; $i++) { $res_memory += $result_cons{$proc_mem_table . "." . $tindex[$i]};}
		$res_memory /= (1024);
		verb("Memory sum : $res_memory"); 
	}
else {
   for (my $i=0; $i< $num_int; $i++) { 
...
if (defined ($o_cpu) ) {
  ...
  if (defined($found_value)) {
	...
	if (defined($o_perf)) {
		if (!defined($perf_output)) {$perf_output="";} else {$perf_output.=" ";}
		$perf_output.= "'cpu_usage'=0%;".$o_cpuL[0].";".$o_cpuL[1];
	}
  }
}
$o_descr =~ s/\|/ or /g ;
